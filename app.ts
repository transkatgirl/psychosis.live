	...
	(globalThis as any).room = new Room(
		...
		(peerId, peer) => {
			...
			peer.pc.ontrack = async (event) => {
				...
				if (jitterBufferTarget && jitterBufferTarget < 0) {
					setReceiverSettings(
						event.receiver,
						REASONABLE_MIN_JITTER_BUFFER
					);
