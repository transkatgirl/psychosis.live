...
function helperMenu() {
	...
	document.body.innerHTML =
		'<h1>psychosis.live</h1><p>This service allows you to stream <em>end-to-end encrypted</em> video from your smartphone into OBS over practically any internet connection, allowing you to stream anything from almost anywhere.</p><p>Unlike similar services, such as <a href="https://vdo.ninja">VDO.Ninja</a>, psychosis.live focuses on <em>both</em> preserving watchability over poor connections and delivering the highest quality possible over good connections.</p><p>It is strongly recommended that you <b>use Google Chrome on both sides of the connection</b> for the best possible experience, as WebRTC implementations vary significantly between browsers.</p>';
	...
	document.body.insertAdjacentHTML(
		...
		'<p>Although this app will not <em>stop you from</em> having multiple receivers, it is a <b>very bad idea</b> to have more than one active receiver in a room, as senders must encode and send video data directly to every receiver.</p><p>If you encounter connection establishment issues, use the advanced URL parameters to configure a TURN server. You can get a free or cheap TURN server using <a href="https://www.expressturn.com">ExpressTURN</a> (not affiliated with this website).</p><details><summary>Known issues</summary>' +
			knownIssuesHTML +
			"</details></br><details><summary>URL parameters (advanced)</summary>" +
			parametersHTML +
