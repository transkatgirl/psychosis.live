...
export default (
	...
	const emitTrack = (
		...
		getSharedMediaPeer(id)?.__trysteroSetRemoteTrackByKey?.(
			key,
			track,
			stream
		);
					...
					iterate(
						...
						async (id, peer) => {
							...
							while (chunkN < chunkTotal) {
								...
								const currentPeer =
									normalizedOptions.sendToPending
										? peerMap[id]
										: activePeerMap[id];
	...
	const [sendStreamMeta, getStreamMeta] =
		makeActionInternal<InternalMediaMeta>(internalNs("stream"));
	...
	const [sendHandshakeData, getHandshakeData] =
		makeActionInternal<DataPayload>(internalNs("hsdata"), {
			sendToPending: true,
			receiveWhilePending: true,
		});
	...
	getHandshakeData((data, id, metadata) => {
		...
		const payload =
			metadata === undefined
				? { data }
				: ({ data, metadata } as HandshakePayload);
	...
	return {
		...
		getPeers: () =>
			fromEntries(
				entries(activePeerMap).map(([id, peer]) => [
					id,
					peer.connection,
				])
