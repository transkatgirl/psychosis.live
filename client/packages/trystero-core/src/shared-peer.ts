...
export class SharedPeerManager {
	...
	bind(
		...
		const proxy: SharedMediaPeer = {
			...
			getOffer: (restartIce?: boolean) =>
				shared.peer.getOffer(restartIce),
			...
			addStream: (stream) => {
				const owners =
					shared.streamOwners.get(stream) ?? new Set<string>();
			...
			__trysteroGetRemoteTrackByKey: (key) =>
				shared.remoteTracksByKey.get(key),
	...
	private dispatchData(shared: SharedPeerState, data: ArrayBuffer): void {
		...
		if (!binding) {
			const pending =
				shared.pendingDataByToken.get(decoded.roomToken) ?? [];
