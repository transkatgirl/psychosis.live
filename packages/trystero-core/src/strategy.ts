...
>({
	...
	> = {};
	const roomRegistrations: Record<
		string,
		Record<string, RoomRegistration>
	> = {};
	...
	return (config: TConfig, roomId: string, callbacks?: JoinRoomCallbacks) => {
		...
		if (!didInit) {
			...
			pool.warmup();
			initPromises = (Array.isArray(initRes) ? initRes : [initRes]).map(
				(value) => Promise.resolve(value)
			);
			didInit = true;
			cleanupWatchOnline = config.manualRelayReconnection
				? noOp
				: watchOnline();
		}
		...
		void all([rootTopicP, selfTopicP]).then(([rootTopic, selfTopic]) => {
			...
			const queueAnnounce = async (
				...
				const currentInterval =
					ctx.announceIntervals[i] ?? announceIntervalMs;
		...
		const { compose } = createPasswordHandshake(
			sharedPassword,
			appId,
			roomId
		);
		...
		const joinedRoom = room(
			...
			() => {
				...
				if (registration?.roomToken) {
					advertiseRoomPresenceToAll(
						appId,
						registration.roomToken,
						false
					);
					...
					if (
						roomIdsByToken[appId] &&
						!keys(roomIdsByToken[appId]).length
					) {
						delete roomIdsByToken[appId];
					}
				}
				...
				entries(ctx.peerStates).forEach(([peerId, state]) => {
					state.answeringExpiryTimer = resetTimer(
						state.answeringExpiryTimer
					);
