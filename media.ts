	...
	stats.forEach((report) => {
		if (report.type == "codec") {
			...
			if (report.mimeType.toLowerCase() == "video/av1") {
				analysis.codecData = adjustCodecData(AV1_ADAPTIVE_DATA, 0.66);
			}
			if (report.mimeType.toLowerCase() == "video/vp9") {
				analysis.codecData = adjustCodecData(VP9_ADAPTIVE_DATA, 0.66);
			}
			if (report.mimeType.toLowerCase() == "video/vp8") {
				analysis.codecData = adjustCodecData(VP8_ADAPTIVE_DATA, 0.33);
			}
			if (report.mimeType.toLowerCase() == "video/h264") {
				analysis.codecData = adjustCodecData(H264_ADAPTIVE_DATA, 0.33);
			}
		}
...
function adaptUp(
	...
	if (width * height >= FHD_PIXELS && framerate < maxFramerate) {
		return [width, height, Math.min(adjustedFramerate, maxFramerate)];
	}
	if (width * height >= HD_PIXELS && framerate < SMOOTH_FPS) {
